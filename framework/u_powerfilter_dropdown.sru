HA$PBExportHeader$u_powerfilter_dropdown.sru
forward
...
type shl_evaltext from statichyperlink within u_powerfilter_dropdown
end type
end forward
...
public function integer of_initialize (ref datawindow adw_dw, ref n_cst_powerfilter a_powerfilter_checkbox, integer a_maxitems, integer ai_maxcol, boolean ab_allowquicksort, string as_defaulttiptext)
end prototypes
...
private function integer of_resize (integer ai_x, integer ai_y);/*
	...
	Copyright $$HEX2$$a9002000$$ENDHEX$$2010 James R. Reese.  All Rights Reserved.
*/
Integer	li_pointerx, li_pointery,li_width,li_height,li_minx,li_miny,li_maxx,li_maxy
...
IF li_height <> this.height or li_width <> this.width THEN
	...
	shl_evaltext.width = li_width - shl_evaltext.x - ii_evalxoffset
	shl_evaltext.y = li_height - shl_evaltext.height - ii_evalyoffset
	...
	CHOOSE CASE dw_powerfilter.dataobject
		CASE 'd_powerfilter'
			dw_powerfilter.object.item.width = dw_powerfilter.width - integer(dw_powerfilter.object.item.x) - 4
			dw_powerfilter.object.timeitem.width = dw_powerfilter.width - integer(dw_powerfilter.object.item.x) - 4
			dw_powerfilter.object.dateitem.width = dw_powerfilter.width - integer(dw_powerfilter.object.item.x) - 4
			dw_powerfilter.object.datetimeitem.width = dw_powerfilter.width - integer(dw_powerfilter.object.item.x) - 4
			dw_powerfilter.object.numericitem.width = dw_powerfilter.width - integer(dw_powerfilter.object.item.x) - 4
			dw_powerfilter.object.t_blank.width = dw_powerfilter.width - integer(dw_powerfilter.object.item.x) - 4
...
ls_PowerTipText = String(is_ReturnParms[ii_colnum].Parm1) + ' ' + string(is_ReturnParms[ii_colnum].parm2)
//messagebox("TEST ColumnFilter first filter only",is_ColumnFilter)
//////////////////////////////////////////////////////////////////
...
ELSE
	...
	ls_PowerTipText = ls_PowerTipText + ' ' + ls_andordisplay + ' ' + string(is_ReturnParms[ii_colnum].Parm3) + ' ' + string(is_ReturnParms[ii_colnum].parm4)
END IF
...
idw_dw.Modify("b_powerfilter"+string(ii_ColNum)+".ToolTip.Tip='"+ls_PowerTipText+"'")
//call function in checkbox to build and apply master filter with changes to this column's filter
//messagebox("TEST?? ColumnFilter",is_ColumnFilter)
iu_powerfilter_checkbox.of_buildfilter(is_ColumnFilter,ii_ColNum)
RETURN 0
...
IF NOT ib_allowquicksort THEN
	IF dw_buttons.rowcount() = 4 THEN
		...
		shl_evaltext.y = shl_evaltext.y - 176
...
ii_evalxoffset = this.width - shl_evaltext.x - shl_evaltext.width
ii_evalyoffset = this.height - shl_evaltext.y - shl_evaltext.height
...
//Evaluation version?  If so, display message hyperlink
IF iu_powerfilter_checkbox.ib_evalversion = TRUE THEN
	shl_evaltext.visible = TRUE
	shl_evaltext.enabled = TRUE
ELSE
	shl_evaltext.visible = FALSE
	shl_evaltext.enabled = FALSE
END IF
RETURN 0
end function
...
this.shl_evaltext=create shl_evaltext
...
string picturename = ".\PFGraphics\checkmark.bmp"
borderstyle borderstyle = styleraised!
...
boolean originalsize = true
string picturename = ".\PFGraphics\resize.bmp"
boolean focusrectangle = false
